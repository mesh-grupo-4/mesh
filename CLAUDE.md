...
### 7. Documentación Swagger/OpenAPI obligatoria (backend)
Todo desarrollo en `backend/` que toque la API REST —endpoint nuevo, cambio de request/response, nuevos códigos de error, cambio de permisos— **debe documentarse en el spec OpenAPI en el mismo cambio o PR**. Un endpoint sin spec se considera incompleto: no cumple el DoD y rompe el build.

- Fuente de verdad: `backend/openapi/` → `openapi.yaml` (raíz) + `paths/*.yaml` (operaciones) + `components/*.yaml` (schemas, parámetros, respuestas, security). **No** editar `src/docs/openapi.bundled.json` a mano: se genera con `npm run docs:bundle`.
- Cada operación declara `operationId` único, `summary`, `tags` ya declarados en la raíz, todos los parámetros de path que usa, al menos una respuesta `2xx` y `401` si es autenticada (solo `GET /api/health` y `/api/docs` son públicos).
- Incluir `example` de request y de response, y los códigos de error con la forma `{ error, code }`.
- Antes de dar el trabajo por terminado, correr `npm run docs:lint` y `npm test`: el test de contrato (`src/docs/contrato.test.ts`) falla si hay endpoints sin documentar o documentación fantasma.
- OpenAPI no cubre WebSockets: los cambios en el canal Socket.io se documentan en `backend/docs/websockets.md`.
...
### Fecha y hora — UTC-3 (RN-105)
Toda fecha y hora que ve el usuario se expresa en **hora de Argentina (UTC-3)**, sin importar la zona del dispositivo ni la del servidor. Offset fijo: Argentina no aplica horario de verano desde 2009.

- Se **almacena y transporta en UTC** (ISO 8601 / `timestamptz`). La conversión ocurre solo en los bordes (RN-106).
- Frontend: usar siempre `frontend/lib/tiempoArg.ts` (`formatearEnArg`, `aCamposArg`, `desdeCamposArg`). **No** llamar a `toLocaleString` / `toLocaleDateString` directamente en componentes.
- Los date pickers nativos trabajan en la zona del dispositivo: convertir con `aCamposArg` al pasarles un valor y con `desdeCamposArg` al leer lo que devuelven.
- La fecha programada de un viaje debe ser **futura**, validada en frontend y backend (`FECHA_PASADA`, RN-107).
...
### API-First
API RESTful documentada bajo **OpenAPI 3.0.3** y servida como **Swagger UI** en `/api/docs`. Todo endpoint nuevo debe tener su spec antes o junto con la implementación (ver *Regla inviolable 7*).

El spec es modular y vive en `backend/openapi/`: `openapi.yaml` como documento raíz (info, servers, security, tags y `paths` por `$ref`), las operaciones en `paths/` y lo compartido en `components/`.

```bash
npm run docs:lint      # valida el spec con @redocly/cli
npm run docs:bundle    # bundlea a src/docs/openapi.bundled.json (lo corren predev, build y pretest)
...
- DoD: Code review + estándares de código + tests unitarios + tests de integración + **spec OpenAPI actualizado si tocó la API** + criterios de aceptación cumplidos + aprobación del PO.
