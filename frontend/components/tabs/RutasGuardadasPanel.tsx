...
export function RutasGuardadasPanel() {
    ...
    <FlatList
        ...
        <View style={[styles.card, { backgroundColor: theme.surface, borderColor: theme.border }]}>
          ...
          >
            ...
            <Text style={[styles.meta, { color: theme.textDim }]}>
              {item.tipo_actividad}
              {item.distancia_planeada_m != null ? ` · ${formatKm(item.distancia_planeada_m)}` : ''}
              {item.tiempo_estimado_seg != null ? ` · ${formatDurationHm(item.tiempo_estimado_seg)}` : ''}
            </Text>
          </Pressable>
      ...
      ListEmptyComponent={
        <Text style={[styles.empty, { color: theme.textDim }]}>
          Todavía no guardaste rutas. Importá una desde la pestaña Explorar o guardala desde un viaje
          compartido.
        </Text>
      }
    />
...
const styles = StyleSheet.create({
  ...
  empty: { fontSize: 15, textAlign: 'center', fontWeight: '600', lineHeight: 22 },
  card: {
    borderWidth: 1,
    borderRadius: 12,
    padding: 14,
